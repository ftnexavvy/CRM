...
export class WorkflowService {
  ...
  async findOne(companyId: string, id: string) { const workflow = await this.workflows.findById(companyId, id); if (!workflow) throw new NotFoundException("Workflow not found"); return workflow; }
  ...
  async generateTasks(companyId: string, actorId: string, workflowId: string, dto: GenerateTasksDto) {
    return this.workflows.transaction(async (tx) => {
      await this.required(tx, companyId, workflowId);
