...
export class WorkflowAutoAssignService {
  ...
  async onboardClient(companyId: string, actorId: string, clientId: string, regenerate = false) {
    ...
    const workflow = await this.prisma.workflow.upsert({
      ...
      update: {
        title: `Client Tasks - ${client.name}`,
  ...
  public resolveDynamicTasks(service: any, config: any, allServicesConfig?: any[], campaignStartDate?: Date, campaignEndDate?: Date): { title: string; departmentName?: string; type: any; dependsOnTitle?: string; isLocked?: boolean; dueDate?: Date }[] {
    ...
    if ((serviceName.includes("graphic") || serviceName.includes("design")) && config) {
      ...
      // DB id than the one Publish depends on → lock never opens.
    ...
    if ((serviceName.includes("photo") || serviceName.includes("shoot")) && config && (config.photoSessions !== undefined || config.videoSessions !== undefined)) {
      ...
      for (let i = 1; i <= photoSessions; i++) {
        ...
        tasks.push({ title: `Photography Session ${i}`, departmentName: "Photography & Video Shoot", type: "GENERIC", dueDate });
      }
      ...
      for (let i = 1; i <= videoSessions; i++) {
        ...
        tasks.push({ title: `Video Shoot Session ${i}`, departmentName: "Photography & Video Shoot", type: "GENERIC", dueDate });
      }
      return tasks;
    }
