...
export class TaskController {
  ...
  @Post(":id/status") @RequirePermissions("Task.Update") async updateStatus(@CurrentUser() user: AuthUser, @Param("id") id: string, @Body("status") status: string) { return this.ok("Task status updated", await this.service.updateStatus(user.companyId, user.id, id, status)); }
